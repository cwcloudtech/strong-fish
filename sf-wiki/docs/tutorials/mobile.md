...
The mobile app can be signed in by [scanning an API key QR code](./api.md#api-keys) instead of typing an email and a password on a phone keyboard.

## Android

There is no release on the Google Play Store yet, but you can download and install the Android app directly, by clicking on _Get the Android app_ in the sidebar.

![android-download](../../static/img/screenshots/android-download.png)

On a desktop it displays a QR code you can scan with your phone; on a phone the download starts straight away.
...
This section is for whoever builds and distributes it. If you are just here to install the app, skip to [Installing a build](#installing-a-build).
